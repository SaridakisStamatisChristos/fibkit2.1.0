...
class FibonacciEngine:
    ...
    @staticmethod
    def _coerce_index(value: Any, *, desc: str, expectation: str) -> int:
        if isinstance(value, bool):
            raise FibonacciError(f"{desc} must be {expectation}, got {type(value)}")
        try:
            return operator.index(value)
        except TypeError:
            raise FibonacciError(f"{desc} must be {expectation}, got {type(value)}") from None

    @staticmethod
    def _require_int(value: Any, *, desc: str) -> int:
        return FibonacciEngine._coerce_index(value, desc=desc, expectation="integer")

    @staticmethod
    def _require_non_negative(value: Any, *, desc: str) -> int:
        result = FibonacciEngine._coerce_index(
            value, desc=desc, expectation="non-negative integer"
        )
        if result < 0:
            raise FibonacciError(f"{desc} must be non-negative integer, got {result}")
        return result

    @staticmethod
    def _require_positive(value: Any, *, desc: str) -> int:
        result = FibonacciEngine._coerce_index(
            value, desc=desc, expectation="positive integer"
        )
        if result <= 0:
            raise FibonacciError(f"{desc} must be positive integer, got {result}")
        return result
